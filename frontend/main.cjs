...
app.whenReady().then(() => {
  ...
  ipcMain.handle('dialog:select-save-file', async (_event, baseDir) => {
    const result = await dialog.showSaveDialog({
      title: 'Save File',
      defaultPath: baseDir || undefined,
      filters: [
        { name: 'TheVine Files', extensions: ['tv'] },
  ...
  ipcMain.handle('dialog:select-any-file', async (_event, defaultPath) => {
    const options = {
      properties: ['openFile']
    };
